...
def train_agent(env, agent, episodes=1000, max_steps=200, epsilon_decay=0.995, min_epsilon=0.1):
    ...
    for episode in range(episodes):
        ...
        for step in range(max_steps):
            ...
            n_new_samples = env.step(torch.cat((agent.location, torch.tensor([env.depth]))), new_loc, agent.speed, agent.sampling_freq)
            if n_new_samples:
                next_prediction = agent.estimate_env(env.env_xy, env.sampled_coords, env.sampled_vals)
                small_grid_mean = agent.make_small_grid(5, env.env_xy, next_prediction.std)
            else:
                next_prediction = agent.current_pred

            reward = agent.compute_reward(agent.current_prediction.std, next_prediction.std)
