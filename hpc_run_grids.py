...
fname = file_ts + '_' + str(index) + '_' + experiment_time_offset + '_' + depth + '_output.pickle'
...
with open(file_path, 'wb') as file_out, open(file_path_fig, 'wb') as file_out_fig:
    for i, spacing in enumerate(grid_spacings):
        ...
        speed = 1.5
        sample_freq = 1
        plain_measurements, plain_sample_coords_list = path.path(dataset, waypoints, ts, speed, sample_freq, data_variable='pH', synoptic=synoptic_sampling)
        ...
        cross_measurements, cross_sample_coords_list = path.path(dataset, cross_waypoints, ts, speed, sample_freq, data_variable='pH', synoptic=synoptic_sampling)
        ...
        for j, rot_dir in enumerate(rot_dirs):
            ...
            for grid_type in grid_types:
                ...
                for kernel_type in kernel_types:
                    ...
                    mdl = ExactGPModel(xy_to_train, sample_values, llh, kernel_type, lengthscale_constraint=length_constraint)
